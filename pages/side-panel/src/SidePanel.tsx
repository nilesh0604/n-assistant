...
const SidePanel = () => {
        ...
        case Actors.SYSTEM:
          switch (state) {
            ...
            case ExecutionState.TASK_RESUME:
              break;
            case ExecutionState.TASK_CLARIFICATION_NEEDED:
              setIsClarificationLoading(false);
              break;
            case ExecutionState.TASK_CLARIFICATION_PROVIDED:
              setIsClarificationLoading(false);
              break;
            case ExecutionState.TASK_CLARIFICATION_CANCELLED:
              setIsClarificationLoading(false);
              break;
            default:
  ...
  const handleClarificationSubmit = useCallback((clarification: string) => {
    ...
    appendMessage({
      actor: Actors.SYSTEM,
      content: '✓ Clarification provided - resuming...',
      timestamp: Date.now(),
    });
  ...
  const handleClarificationCancel = useCallback(() => {
    ...
    appendMessage({
      actor: Actors.SYSTEM,
      content: '✗ Clarification cancelled - task will continue with original intent',
      timestamp: Date.now(),
    });
  ...
  const setupConnection = useCallback(() => {
    ...
    try {
      ...
      portRef.current.onMessage.addListener((message: any) => {
        ...
        } else if (message && message.type === EventType.TASK_CLARIFICATION_NEEDED) {
          // Handle task clarification request from backend
          console.log('🔍 Clarification event received:', message);
          setClarificationMessage(message.message || 'I need more information to complete this task accurately. Please provide additional details about what you want me to do.');
          setShowClarificationDialog(true);
          console.log('🔍 Setting showClarificationDialog to true');
          appendMessage({
            actor: Actors.SYSTEM,
            content: '⏸️ Task paused - clarification needed',
            timestamp: Date.now(),
          });
        } else {
  ...
  const handleReplay = async (historySessionId: string): Promise<void> => {
    try {
      ...
      if (!historyData) {
        appendMessage({
          actor: Actors.SYSTEM,
          content: `No history found for session: ${historySessionId.substring(0, 20)}`,
          timestamp: Date.now(),
        });
        return;
      }
      ...
      appendMessage({
        actor: Actors.SYSTEM,
        content: `Starting replay for task: ${historyData.task}`,
        timestamp: Date.now(),
      });
      setIsReplaying(true);
    } catch (err) {
    ...
    try {
      ...
      if (command.startsWith('/replay ')) {
        ...
        if (parts.length !== 2) {
          appendMessage({
            actor: Actors.SYSTEM,
            content: 'Invalid arguments for replay command',
            timestamp: Date.now(),
          });
          return true;
        }
        ...
        if (audioChunksRef.current.length > 0) {
          ...
          reader.onloadend = () => {
            ...
            try {
              ...
              appendMessage({
                actor: Actors.SYSTEM,
                content: 'Speech processing failed',
                timestamp: Date.now(),
              });
              setIsRecording(false);
              setIsProcessingSpeech(false);
  ...
  return (
    <div>
      <div
        ...
        <header className="header relative w-full flex justify-between items-center p-4">
          ...
          <div className="header-icons">
            ...
            <button
              ...
              className={`header-icon ml-auto ${isDarkMode ? 'text-sky-400 hover:text-sky-300' : 'text-sky-400 hover:text-sky-500'} cursor-pointer`}
              aria-label='Settings'
              tabIndex={0}>
              <FiSettings size={20} />
            </button>
            ...
            {hasConfiguredModels === false && (
              <div
                ...
                <div className="max-w-md text-center">
                  <img src="/icon-128.png" alt="N-assistant Logo" className="mx-auto mb-4 size-12" />
                  <h3 className={`mb-2 text-lg font-semibold ${isDarkMode ? 'text-sky-200' : 'text-sky-700'}`}>
                    Welcome to N-assistant
                  </h3>
                  <p className="mb-4">Start by configuring your AI provider in Settings, then try a simple task like "search for cats"</p>
                  <button
                    onClick={() => chrome.runtime.openOptionsPage()}
                  ...
                  <div className="mt-4 text-sm opacity-75">
                    <a
                      href="https://github.com/nilesh0604/local-ai-assistant?tab=readme-ov-file#-quick-start"
                      target="_blank"
                      rel="noopener noreferrer"
