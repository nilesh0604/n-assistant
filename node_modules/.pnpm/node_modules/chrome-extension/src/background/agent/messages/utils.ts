...
/**
 * Enhanced JSON extraction with progressive fallback strategies
 * @param content - The string content that potentially contains JSON.
 * @param schema - Optional Zod schema for validation (future enhancement)
 * @returns Parsed JSON object
 * @throws Error if JSON parsing fails after all fallback strategies
 */
export function extractJsonFromModelOutput(content: string, schema?: any): Record<string, unknown> {
  const strategies = [
    () => tryDirectParse(content),
    () => tryMarkdownExtract(content),
    () => tryJsonRepair(content),
    () => tryAlternativeFormats(content),
  ];

  for (const strategy of strategies) {
    try {
      const result = strategy();
      if (result && isValidJsonObject(result)) {
        return result;
      }
    } catch (error) {
  ...
  throw new ResponseParseError(`Could not extract JSON from model output after all fallback strategies`);
}

/**
 * Strategy 1: Direct JSON parsing with existing format handling
 */
function tryDirectParse(content: string): Record<string, unknown> {
  let processedContent = content;

  // Handle Llama's tool call format first
  if (processedContent.includes('<|tool_call_start_id|>')) {
    processedContent = extractLlamaToolCallContent(processedContent);
  }

  // Handle Llama's python tag format
  if (processedContent.includes('<|python_tag|>')) {
    processedContent = extractLlamaPythonTagContent(processedContent);
  }

  // If content is wrapped in code blocks, extract just the JSON part
  if (processedContent.includes('```')) {
    processedContent = extractFromCodeBlocks(processedContent);
  }
...
/**
 * Strategy 2: Extract from markdown code blocks with better handling
 */
function tryMarkdownExtract(content: string): Record<string, unknown> {
  ...
  for (const pattern of patterns) {
    ...
    for (const match of matches) {
      try {
        const jsonContent = match[1]?.trim() || match[0]?.trim();
  ...
  throw new Error('No valid JSON found in markdown');
}

/**
 * Strategy 3: Repair common JSON formatting issues
 */
function tryJsonRepair(content: string): Record<string, unknown> {
  let repairedContent = content;

  // Remove think tags if present
  repairedContent = removeThinkTags(repairedContent);

  // Common JSON repair operations
  repairedContent = repairJsonCommonIssues(repairedContent);
  ...
  throw new Error('Could not repair JSON format');
}

/**
 * Strategy 4: Try alternative LLM-specific formats
 */
function tryAlternativeFormats(content: string): Record<string, unknown> {
  // Handle models that output JSON with extra text
  ...
  for (const line of lines) {
    if (line.trim().startsWith('{')) {
      jsonCandidate += line;
    } else if (jsonCandidate && line.trim()) {
      jsonCandidate += line;
    }
  ...
  throw new Error('No alternative JSON format found');
}

/**
 * Helper functions for content extraction
 */
function extractLlamaToolCallContent(content: string): string {
  ...
  const processedContent = content.substring(startIndex, endIndex).trim();
  const toolCall = JSON.parse(processedContent);

  if (toolCall.parameters) {
    const parametersJson = JSON.parse(toolCall.parameters);
    return JSON.stringify(parametersJson);
  }

  throw new Error('Tool call structure does not contain parameters');
}

function extractLlamaPythonTagContent(content: string): string {
  ...
  if (pythonCall.parameters && pythonCall.parameters.output) {
    if (typeof pythonCall.parameters.output === 'string') {
      try {
        ...
        return JSON.stringify({ output: pythonCall.parameters.output });
      }
    }

    return JSON.stringify(pythonCall.parameters);
  }

  throw new Error('Python tag structure does not contain valid parameters');
}

function extractFromCodeBlocks(content: string): string {
  const parts = content.split('```');
  let processedContent = parts[1];

  // Remove language identifier if present
  if (processedContent?.startsWith('json')) {
    processedContent = processedContent.substring(4).trim();
  } else if (processedContent?.startsWith('javascript')) {
...
function repairJsonCommonIssues(content: string): string {
