...
const fingerprintService = new ElementFingerprintService();

/**
 * Generate fingerprints for interactive elements in the DOM tree
 */
function generateFingerprintsForInteractiveElements(element: DOMElementNode): void {
  if (element instanceof DOMElementNode && element.isInteractive) {
    element.fingerprint = fingerprintService.generateFingerprint(element);
  }

  // Recursively process children
  for (const child of element.children) {
    if (child instanceof DOMElementNode) {
      generateFingerprintsForInteractiveElements(child);
    }
  }
}
...
export async function getClickableElements(
  ...
  // Generate fingerprints for interactive elements after tree construction
  generateFingerprintsForInteractiveElements(elementTree);
